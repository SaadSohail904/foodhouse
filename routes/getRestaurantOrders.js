  ...
  try{
    ...
    if(!validated.error){
        ...
        if(restaurantResults.length){
          ...
          on m.food_item_id = f.id inner join restaurants r on r.id = f.restaurant_id inner join user u on u.id = r.user_id where o.restaurant_id = ${restaurantResults[0].id}`);
        }
