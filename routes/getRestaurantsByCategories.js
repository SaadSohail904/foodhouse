  ...
  try{
    ...
    if(!validated.error){
        ...
        if(req.body.categories.length){
          restaurantResults = await functions.runQuery(`Select distinct r.id as id, r.*, u.* from restaurants r inner join user u on u.id = r.user_id left join restaurant_categories_mapper m on r.id = m.restaurant_id left join
