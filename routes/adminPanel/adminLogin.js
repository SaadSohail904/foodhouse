      ...
      con.beginTransaction(async function () {
        try {
          if (!validated.error) {
            ...
            let user = await functions.runTransactionQuery(`SELECT id, name, email, password, user_id FROM admin WHERE email = "${req.body.email}"`, con)
            if (user.length) {
              ...
              if (req.body.password != user[0].password){
                ...
                let query = `Insert into authtoken (token , user_id, start_date, end_date) values("${token}" ,${user[0].user_id},
