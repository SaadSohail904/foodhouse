  ...
  try{
    ...
    if(!validated.error){
      ...
      if(customerResults.length){
        ...
        inner join restaurants r on f.restaurant_id = r.id inner join cart c on c.id = m.cart_id where c.id in (Select id from cart where customer_id = ${customerResults[0].id})`);
