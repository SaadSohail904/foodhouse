...
router.post('/', async function (req, res, next) {
  try{
    ...
    if(!validated.error){
        var insertionResults = await functions.runQuery(`Delete from favourites_user_food_mapper where user_id = ${req.body.user_id} && food_item_id = ${req.body.food_item_id}`);
