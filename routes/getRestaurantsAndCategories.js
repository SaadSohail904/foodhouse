...
router.get('/', async function (req, res, next) {
  try{
    ...
    if(!validated.error){
      var restaurantResults = await functions.runQuery(`Select * from restaurants inner join user on restaurants.user_id = user.id  where name like "%${req.query.filter}%"`);
